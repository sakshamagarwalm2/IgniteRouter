...
This is exactly the problem [IgniteRouter](https://github.com/IgniteRouterAI/IgniteRouter) solves.

## IgniteRouter: Smart Routing for Agents

IgniteRouter is an open-source local proxy that sits between your agent and 55+ LLM models across 9 providers. It analyzes every request across 15 dimensions and routes it to the cheapest model that can handle it — in under 1ms, entirely locally.

```
Your Agent → IgniteRouter (localhost:8402) → Best model for the job
...
- **IgniteRouter: ~$20/month**

### How It Works

```bash
npx @igniterouter/igniterouter    # Install and start (generates wallet automatically)
```
...
- **Budget caps** — set a max spend per session, IgniteRouter gracefully downgrades when budget runs low
...
IgniteRouter is open source, runs locally, and takes 30 seconds to set up:
...
_IgniteRouter is built by [IgniteRouter](https://IgniteRouter.ai). Source code: [github.com/IgniteRouterAI/IgniteRouter](https://github.com/IgniteRouterAI/IgniteRouter)_
