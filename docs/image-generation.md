...
Generate and edit images via IgniteRouter's image API with x402 micropayments — no API keys, pay per image.
...
IgniteRouter runs a local proxy on port `8402` that handles x402 payments automatically. Point any OpenAI-compatible client at it:
...
OpenAI-compatible endpoint. Route via IgniteRouter proxy (`http://localhost:8402`) for automatic x402 payment handling.
...
Edit an existing image using AI. Route via IgniteRouter proxy (`http://localhost:8402`) for automatic x402 payment handling.
...
| ------------------ | ---------------------------------- | ---------------------------------------------- |
| Local file path    | `"/Users/me/photo.png"`            | Absolute path — IgniteRouter reads the file      |
| Home-relative path | `"~/photo.png"`                    | Expands `~` to home directory                  |
| HTTP/HTTPS URL     | `"https://example.com/photo.png"`  | IgniteRouter downloads the image automatically   |
...
const client = new OpenAI({
  apiKey: "IgniteRouter", // any non-empty string
...
If you're using IgniteRouter as a library:

```typescript
import { startProxy } from "@igniterouter/igniterouter";

const proxy = await startProxy({ walletKey: process.env.IgniteRouter_WALLET_KEY! });
...
```typescript
// IgniteRouter reads the file for you — no base64 encoding needed
...
When using IgniteRouter with OpenClaw, generate and edit images directly from any conversation:
...
- **Local image caching** — All images (generated and edited) are cached locally at `~/.openclaw/IgniteRouter/images/` and served via `http://localhost:8402/images/`. Both base64 data URIs and HTTP URLs from upstream are downloaded and replaced with localhost URLs.
...
- **Image editing** — The `/v1/images/image2image` endpoint currently supports `openai/gpt-image-1` (default). The `image` and `mask` fields accept local file paths (`~/photo.png`, `/abs/path.png`), HTTP/HTTPS URLs, or base64 data URIs. IgniteRouter handles file reading and URL downloading automatically. Supported formats: PNG, JPG/JPEG, WebP.
