...
Quick solutions for common IgniteRouter issues.

> Need help? [Open a Discussion](https://github.com/IgniteRouterAI/IgniteRouter/discussions) or check [existing issues](https://github.com/IgniteRouterAI/IgniteRouter/issues).
...
cat ~/.openclaw/extensions/IgniteRouter/package.json | grep version
...
### "Unknown model: igniterouter/auto" or "Unknown model: auto"

Plugin isn't loaded or outdated. **Don't change the model name** — `igniterouter/auto` is correct.

**Fix:** Update to v0.3.21+ which handles both `igniterouter/auto` and `auto` (OpenClaw strips provider prefix). See [How to Update](#how-to-update).

### "No API key found for provider IgniteRouter"
...
### "Config validation failed: plugin not found: IgniteRouter"
...
Wallet needs funding. IgniteRouter accepts **USDC** (not SOL or ETH) on either chain.
...
This is a **false positive**. IgniteRouter legitimately:

1. Reads `IgniteRouter_WALLET_KEY` from environment (for authentication)
2. Sends authenticated requests to IgniteRouter API (for x402 micropayments)
...
OpenClaw's security scanner may flag IgniteRouter with:
...
IgniteRouter reads `IgniteRouter_WALLET_KEY` to sign x402 payment transactions — this is required and intentional:
...
- Source code is [MIT licensed and fully auditable](https://github.com/IgniteRouterAI/IgniteRouter)
...
As of v0.4.1, IgniteRouter automatically detects and reuses an existing proxy on the configured port instead of failing with `EADDRINUSE`. You should no longer see this error.
...
```bash
npx @igniterouter/igniterouter@latest
openclaw gateway restart
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/IgniteRouterAI/IgniteRouter/main/scripts/reinstall.sh | bash
openclaw gateway restart
```
