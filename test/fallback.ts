 ...
 * IgniteRouter correctly falls back to the next model in the chain.
 *
 * Usage:
...
async function runTests() {
  ...
  // Test 6b: Delisted explicit model IDs should alias to a live replacement
  {
    ...
    const res = await fetch(`${proxy.baseUrl}/v1/chat/completions`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        model: "IgniteRouter/xai/grok-code-fast-1",
  ...
  // This proves the projected-cost check, not just a trailing "already exceeded" check.
  {
    ...
    assert(
      res2.headers.get("X-IgniteRouter-Cost-Cap-Exceeded") === "1",
      `X-IgniteRouter-Cost-Cap-Exceeded header set: ${res2.headers.get("X-IgniteRouter-Cost-Cap-Exceeded")}`,
    );
  ...
  // Uses igniterouter/auto (routing profile) so IgniteRouter chose the model and can downgrade.
  // cap=$0.0015: first request succeeds ($0.001 minimum cost), second is downgraded ($0.0005 left).
  {
    ...
    // First request: igniterouter/auto routing profile, succeeds, accumulates cost ($0.001 min)
    ...
    assert(
      res2.headers.get("x-IgniteRouter-budget-downgrade") === "1",
      `x-IgniteRouter-budget-downgrade header set: ${res2.headers.get("x-IgniteRouter-budget-downgrade")}`,
    );
    assert(
      res2.headers.get("x-IgniteRouter-budget-mode") === "downgraded",
      `x-IgniteRouter-budget-mode is downgraded: ${res2.headers.get("x-IgniteRouter-budget-mode")}`,
    );
  ...
  // substituting it with free model would be deceptive regardless of task complexity.
  {
    ...
    assert(
      res2d.headers.get("X-IgniteRouter-Budget-Mode") === "blocked",
      `X-IgniteRouter-Budget-Mode is blocked: ${res2d.headers.get("X-IgniteRouter-Budget-Mode")}`,
    );
  ...
  // Test 10c: maxCostPerRun graceful mode — tools request blocked when budget can't afford any model
  {
    ...
    assert(
      res.headers.get("X-IgniteRouter-Budget-Mode") === "blocked",
      `X-IgniteRouter-Budget-Mode is blocked: ${res.headers.get("X-IgniteRouter-Budget-Mode")}`,
    );
